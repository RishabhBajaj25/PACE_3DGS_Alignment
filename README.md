...
This branch is for aligning R2Gaussian and Gaussian Splat using manual points picked in CloudCompare. It generates files that `utils/apply_transform.py` can use to apply the transformation to the Gaussian splats. The transformation is applied to the Gaussian splats in the [`gaussian_splatting_lightning`](https://github.com/RishabhBajaj25/gaussian-splatting-lightning) repository. The code is not yet fully functional but serves as a starting point for future work.
